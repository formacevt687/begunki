class Player {
    constructor(x, y) {
        ...
        this.coyoteTimer = 0;
        this.jumpBuffer = 0; // только для pre-jump
        this.animFrame = 0;
        this.animTimer = 0;
        this.hurtTimer = 0;
        this.victoryTimer = 0;
    ...
    respawn() {
        if (this.lastCheckpoint) {
            ...
            this.y = this.lastCheckpoint.y - this.height;
        } else {
            this.x = this.spawnX;
            this.y = this.spawnY;
        }
    ...
    takeDamage() {
        ...
        Audio.sfxHurt();
        Particles.hurtBurst(this.x + this.width / 2, this.y + this.height / 2);
        Storage.addDeath();
        return this.lives <= 0;
    }
    ...
    update(level) {
        ...
        const jumpJustReleased = this.prevJumpHeld && !jumpHeld;
        this.prevJumpHeld = jumpHeld;

        // Запоминаем нажатие в буфер (для pre-jump)
        if (jumpPressed) {
            this.jumpBuffer = 5; // 5 кадров = 83мс
        }

        // Проверяем прыжок из буфера
        if (this.jumpBuffer > 0) {
            if (this.onGround || this.coyoteTimer > 0) {
                // Обычный прыжок — МГНОВЕННО
                ...
                // Двойной прыжок — МГНОВЕННО
                this.vy = -CONFIG.DOUBLE_JUMP_POWER;
                this.canDoubleJump = false;
                this.jumpBuffer = 0;
                Audio.sfxDoubleJump();
                Particles.burst(this.x + this.width / 2, this.y + this.height, 10, 
                    ['#ffffff', '#aaddff', '#88bbff'], 
                    { speedMin: 1, speedMax: 3, lifeMin: 15, lifeMax: 25, gravity: 0.05 });
            }
        }

        // Variable jump height
        if (jumpJustReleased && this.vy < -4) {
            this.vy = this.vy * 0.5;
        }
        ...
        if (inputX !== 0) {
            this.vx += inputX * CONFIG.PLAYER_ACCEL;
            this.facing = inputX;
        ...
        // === COYOTE TIME — УВЕЛИЧЕН ДО 8 КАДРОВ ===
        if (this.onGround) {
            this.coyoteTimer = 8; // 133мс — прощает микро-отрывы
        ...
        if (this.onMovingPlatform) {
            this.x += this.onMovingPlatform.deltaX;
            this.y += this.onMovingPlatform.deltaY;
        }

        this.x += this.vx;
        level.resolvePlayerHorizontal(this);

        const wasOnGround = this.onGround;
        this.onGround = false;
        this.onMovingPlatform = null;
        this.y += this.vy;
        level.resolvePlayerVertical(this);
    ...
    draw(ctx, camX, camY) {
        ...
        if (this.state === 'run') {
            bodyOffsetY = Math.abs(Math.sin(this.animFrame * Math.PI / 2)) * 2;
        ...
        ctx.beginPath(); ctx.ellipse(w / 2, h + 2, w / 2, 3, 0, 0, Math.PI * 2); ctx.fill();

        // Ноги
        ctx.fillStyle = CONFIG.COLORS.PLAYER_PANTS;
        if (this.state === 'run') {
            ctx.fillRect(4, h - 10 + bodyOffsetY, 6, 10 - bodyOffsetY);
            ctx.fillRect(w - 10, h - 10 + bodyOffsetY, 6, 10 - bodyOffsetY);
            ctx.fillStyle = CONFIG.COLORS.PLAYER_BOOTS;
            ctx.fillRect(4 + legOffset * 0.3, h - 4, 6, 4);
            ctx.fillRect(w - 10 - legOffset * 0.3, h - 4, 6, 4);
        } else if (this.state === 'jump' || this.state === 'doubleJump') {
            ctx.fillRect(3, h - 8, 7, 8); ctx.fillRect(w - 10, h - 8, 7, 8);
            ctx.fillStyle = CONFIG.COLORS.PLAYER_BOOTS;
            ctx.fillRect(3, h - 3, 7, 3); ctx.fillRect(w - 10, h - 3, 7, 3);
        } else {
            ctx.fillRect(5, h - 10 + bodyOffsetY, 6, 10 - bodyOffsetY);
            ctx.fillRect(w - 11, h - 10 + bodyOffsetY, 6, 10 - bodyOffsetY);
            ctx.fillStyle = CONFIG.COLORS.PLAYER_BOOTS;
            ctx.fillRect(5, h - 4, 6, 4); ctx.fillRect(w - 11, h - 4, 6, 4);
        }

        // Тело
        ctx.fillStyle = CONFIG.COLORS.PLAYER_SHIRT;
        ctx.fillRect(3, 10 + bodyOffsetY, w - 6, h - 20);
        ...
        if (this.state === 'run') {
            ctx.fillRect(0, 14 + bodyOffsetY - legOffset * 0.3, 4, 10);
            ctx.fillRect(w - 4, 14 + bodyOffsetY + legOffset * 0.3, 4, 10);
        } else if (this.state === 'jump' || this.state === 'doubleJump') {
            ctx.fillRect(0, 6 + bodyOffsetY, 4, 10);
            ctx.fillRect(w - 4, 6 + bodyOffsetY, 4, 10);
        } else if (this.state === 'hurt') {
            ctx.fillRect(0, 18 + bodyOffsetY, 4, 10);
            ctx.fillRect(w - 4, 18 + bodyOffsetY, 4, 10);
        } else {
            ctx.fillRect(0, 14 + bodyOffsetY, 4, 12);
            ctx.fillRect(w - 4, 14 + bodyOffsetY, 4, 12);
        }

        // Голова
        ctx.fillStyle = CONFIG.COLORS.PLAYER_SKIN;
        ctx.fillRect(4, 2 + bodyOffsetY, w - 8, 10);
        ctx.fillStyle = CONFIG.COLORS.PLAYER_HAIR;
        ctx.fillRect(4, 0 + bodyOffsetY, w - 8, 4);
        ctx.fillRect(3, 2 + bodyOffsetY, 2, 3);
        ...
        ctx.fillRect(2, -2 + bodyOffsetY, w - 4, 3);
        ctx.fillRect(6, -5 + bodyOffsetY, w - 12, 3);
        ctx.fillStyle = '#5a3818';
        ctx.fillRect(2, 0 + bodyOffsetY, w - 4, 1);
